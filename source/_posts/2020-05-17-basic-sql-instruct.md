...
### 什么是SQL
 ...
 Records or Rows发音就是单个字母拼读，是结构化查询语言的缩写（structured query language）。
...
- 看上去简单，却是一种强有力的语言，灵活组合他的语言元素，可以进行非常复杂和高级数据库操作。

不同的DBMS实现的SQL都不完全一样。但存在一个标准委员会，在定义可供所有DBMS使用的sql，
