...
对应页面，看调试窗口中所有 html dom 的结构，可以在 dom 树中：
...
解决：只需要，在对应节点 dom 右键点选`:hover`即可. 这样就可以把页面固定住，对当前状态下的配色等参数进行调整。
当然，也可以对某个页面元素设置多个状态：
...
想在页面做改变的时候，能够停住调试。chrome 里面有对应的方法。
办法是控制整个页面的 div，一有改动我们就把变化停掉。当然，也可以细腻地控制，只控制某个节点的 div 元素的变动。
在 dom 节点右键 breake on 有三个选项：
...
- node removal 节点移除的时候，
  ...
  停掉监控的方法：element/DOM Breakpoints，找到该节点的断点：勾掉，断点就暂时不起作用。当然也可以右键删除断点，或者删除所有断点。

**06 样式快速调试与修改**

左边选取 dom 元素，右侧就可调整其对应样式。
...
通过捕捉动画瞬间，还是比较麻烦，可以直接在样式`.hov`里勾选相应状态。
...
查看元素样式，可以进入`style`查看，要上下滚动查找，还要排除被杠掉的样式。
...
当然除了 click 事件，还有 focus、focusin、focusout、keydown、load 等各种各样的事件，可以快速定位到各自事件上做代码查看。这个功能是非常有用的，因为有时后不知道点击后，调用的 click 代码是哪边的，就可以通过这种方法快递定位到对应的函数里面来。
...
根据 10 的方法，找到被调用的事件方法在源码。这仅仅是找到，得到的代码被压缩成一行，可读性差。要想查阅，还需按下方大括号`pretty print`来展开所有代码。前提是源码没有做混淆。
css 代码的查看也是一样。
...
在浏览器中调样式，只是模拟修改。要想改变本地源码还得拷贝到本地文件。
有时这是不满足大量修改的需求的。希望可以在浏览器修改，本地就自动保存了。
方法是有的：
...
进入后，看到界面上有时间轴，以及如何刷新加载的提示。请求之后，可以看到当前网络的所有请求数据流；看到每一段的请求，请求了哪些资源。
...
页面上没有使用到的 CSS、JavaScript，可以快速监控。开发会引用他人 CSS 资源库、类包，实际只用了很少的东西，加载却全部加载。这时就可做网页优化，把不使用的内容去掉。问题来了：

> 如何知道哪些有用，哪些没有？（当然，前期开发的时候就要把关，不引入没用的代码）

打开 more tools/ coverage 工具。(coverage 理解为：覆盖面积或覆盖率)
他已经给出了各文件夹使用率数据，并有颜色区分，非常直观。甚至，可以点进文件，看具体代码段的使用详情。
...
大的站点会适配 PC，pad，手机等不同终端设备，给出不同的显示效果。我们调试时，配齐所有外部重终端设备话，不大现实，在 pc 端模拟各终端的设备访问即可。
